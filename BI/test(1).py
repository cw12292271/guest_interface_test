...
#from settings.setting import redis_str, CELERY_REDIS_DB
#import redis
#from utils.utils import division
import json
from sqlalchemy import func
# from settings.local_setting import mysql_str
import pandas as pd
import json
import numpy
...
# redis_url = "{}/{}".format(redis_str, CELERY_REDIS_DB)
# redis = redis.from_url(redis_url, charset="utf-8", decode_responses=True)
...
range
# query = db.query(Statis).filter(Statis.cid == cid, Statis.statis_type == 1,
#                                     Statis.created > start, Statis.created <= end, Statis.user_id == 0)
# for statis in query:
#     # result.append(statis.to_json())
#     print(statis.to_json()["created"])
# print(result)

# for each in query.all():
#     dictret = dict(each.__dict__)
...
class BaseStatis(object):
    ...
    def _机器人消息量_匹配数_未匹配数(self, table):
        ...
        robot_df = bot_msg["raw"].str.extract(
            """"from_code"(: 8,|: 17,|: 21,|: 9,|: 20,|: 16,|: 2,|: 3,|: 19,|: 7,
            |: 22,|: 1,|: 15,|: 10,|: 18,|: 5,|: 25,|: 6,|: 4,|: 13,|: 23,|: 12,
            |: 26,|: 27,|: 28,)""",
            expand=True)
    ...
    def _接待时长统计(self, table):
        ...
        for sid, df_logs in table.groupby("sid"):  # 按照 sid 聚合聊天日志
            ...
            for idx, log in df_logs.iterrows():
                ...
                print(log.raw)
                raw = json.loads(log.raw)
                if raw.get("text") and raw.get("from") == "kf" and raw.get(
                        "is_sys_send") not in ["1", 1]:
    ...
    def _时长统计(self, df, idx, _in='', _out=''):
        if idx == "action":
            df = df.loc[
                (df["content"] != "强行登录成功") & (df["content"] != "重复登入注销")]
            登录 = self._series_index(
                df.loc[(df[idx] == _in) & (df["content"] == "成功")], 0)
            注销 = self._series_index(
                df.loc[(df[idx].str.contains(_out)) & (df["content"] == "成功")],
                -1)
        else:
        ...
        today_start, today_end = self._get_today_date(
            self.statis_type, 'start'), self._get_today_date(self.statis_type,
                                                             'end')
        if not login_time:
            if not login:
                login = today_start
    ...
    def _客服状态切换时间(self, table, kf_status_table):
        """客服切换状态"""

        df = table.loc[table["action"] == "客服上下线"]
        if not df.size:
            result = {}
            if kf_status_table.size:
                kf_content = kf_status_table["value"].values[0]
                kf_content = json.loads(kf_content)
                kf_status = [each["label"] for each in kf_content]
                already_exist = list(set(kf_status) - set(result.keys()))
                result.update({each: 0 for each in already_exist})

            return {"客服状态切换时间": json.dumps(result, ensure_ascii=False)}
    ...
    def _独立接待量(self, table):
        count = 0
        table = table.loc[(table.user_id.notnull()) & (table.creator == 1)]
        for ori_session, dataframe in table.groupby("ori_session"):
            count += 1
        return count

    def _首次响应超时会话量(self, table):
        df = table.loc[(table.last_session.isnull())
                       & (table.user_id.notnull()) & (table.stop_way == 10)]
        return self._count(df)
    ...
    def _get_today_date(self, statis_type, start='start'):
        if start == 'start':
            if statis_type:
                ...
                return (
                               self.now or datetime.datetime.now()) - datetime.timedelta(
                    hours=1)
        elif start == 'end':
            if statis_type:
                return datetime.datetime.combine(
...
class StatisCompany(BaseStatis):
    def __init__(self, df, now):
        super(StatisCompany, self).__init__(df)
        self.now = now
    ...
    def statis_session(self):
        ...
        # result["独立接待量"] = self._独立接待量(table)
        # result["首次响应超时会话量"] = self._首次响应超时会话量(table)
        return result
...
mysql_str = 'mysql+pymysql://root:ZEGAhuDP@webot2dingdang.c1t9kbuexy3c.rds.cn-north-1.amazonaws.com.cn:3306/webot2-t6?charset=utf8mb4'


# mysql_str = 'mysql+pymysql://root:123456@192.168.3.120/webot2?charset=utf8mb4'

# mysql_str = 'mysql+pymysql://test:test@webot210.c1t9kbuexy3c.rds.cn-north-1.amazonaws.com.cn:3306/webot2?charset=utf8mb4'

# redis_str = 'redis://192.168.3.120:6379/9'

# mysql_str = 'mysql+pymysql://test:test@webot2.c1t9kbuexy3c.rds.cn-north-1.amazonaws.com.cn:3306/webot2x0524?charset=utf8mb4'
...
def get_db(local=True):
    ...
    返回sqlalchemy实例
    """
    engine = create_engine(mysql_str if local else webot2_mysql_str,
...
def get_redis():
    return redis.from_url(redis_str, charset="utf-8", decode_responses=True)


start = "2018-08-01 00:00:00"
end = "2018-08-10 00:00:00"
cid = 1
...
df = {
    ...
    'question_tag': pd.read_sql_query(
        """select * from question_tag where cid = {}""".format(cid),
        mysql_str),

    'users': pd.read_sql_query(
        """select * from user where cid = {}""".format(cid), mysql_str),
    ...
    "kf_status": pd.read_sql_query(
        "select * from company_setting where cid = 1 and  name = 'kf_status'",
        mysql_str)
...
# 客服
kdf = {
    table: df[table].loc[df[table].user_id == 105]
...
kdf["kf_status"] = df["kf_status"]

now = datetime.datetime.strptime("2018-08-07 00:00:00", "%Y-%m-%d %H:%M:%S")
company = StatisCompany(df, now=now).statistic()
from pprint import pprint
